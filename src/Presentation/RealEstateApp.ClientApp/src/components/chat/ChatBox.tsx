...
export const ChatBox: React.FC<ChatBoxProps> = ({
  ...
  return (
    <div className="flex flex-col h-[500px] bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
      ...
      {/* Header */}
      <div className="p-3.5 bg-slate-900 dark:bg-slate-950 text-white flex items-center justify-between border-b dark:border-slate-800">
      ...
      {/* Message History */}
      <div className="flex-1 p-4 overflow-y-auto space-y-3 bg-slate-50/50 dark:bg-slate-950/50 custom-scrollbar">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-slate-400 text-center px-4 space-y-2">
            ...
            <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">Inicia la conversación</p>
                ...
                <div
                  className={`max-w-[80%] rounded-2xl px-4 py-2.5 text-xs shadow-sm ${
                    isMe
                      ...
                      : 'bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-200 border border-slate-200/80 dark:border-slate-700 rounded-bl-sm'
                  }`}
                >
      ...
      <form onSubmit={handleSend} className="p-3 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 flex items-center gap-2">
        <input
          ...
          className="flex-1 px-3.5 py-2 text-xs rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500"
        />
        <button
          type="submit"
