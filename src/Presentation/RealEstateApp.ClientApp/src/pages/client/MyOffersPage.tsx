...
export const MyOffersPage: React.FC = () => {
  ...
  const submitReview = async () => {
    ...
    try {
      ...
      alert(err.response?.data?.error || "Error al enviar la calificación.");
    } finally {
  ...
  const handleAcceptCounterOffer = async (offerId: number) => {
    ...
    try {
      ...
      await loadOffers();
      alert("¡Contraoferta aceptada! La propiedad ha sido marcada como VENDIDA. Ya puedes calificar al agente desde Mis Ofertas.");
    } catch (err: any) {
    ...
    <div className="space-y-6">
      ...
      {reviewOffer && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-6 sm:p-8 space-y-5 border border-slate-200">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  #{reviewOffer.propertyCode || reviewOffer.propertyId} · Venta por {formatCurrencyRD(reviewOffer.amount)}
                </p>
              </div>
              <button
            ...
            {alreadyReviewed ? (
              <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-2xl text-center space-y-1">
                ...
                <p className="text-sm font-bold text-emerald-800">¡Ya calificaste a este agente!</p>
                <p className="text-[11px] text-emerald-700">Solo se permite una reseña por transacción.</p>
              </div>
            ) : (
              <>
                <div className="space-y-3">
                  ...
                  <div className="flex flex-col items-center gap-2 py-2 bg-slate-50 rounded-2xl border border-slate-100">
                ...
                <div className="space-y-2">
                  <label htmlFor="review-comment" className="block text-[11px] font-extrabold uppercase tracking-wider text-slate-500">
                    Comentario (opcional)
                  </label>
                  <textarea
                    id="review-comment"
                    ...
                    className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-800 placeholder:text-slate-400 focus:border-brand-500 focus:ring-1 focus:ring-brand-500 focus:outline-none"
                  />
                </div>
